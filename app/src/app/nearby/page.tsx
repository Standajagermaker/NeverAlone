    ...
    <Shell>
      <section className="mx-auto max-w-6xl px-6 py-12">
        <div className="flex flex-wrap items-end justify-between gap-4">
          ...
          <Link href="/profile#listing" className="rounded-full bg-white px-6 py-3 font-semibold text-slate-950 hover:bg-cyan-100">
            Create my listing
          </Link>
        </div>
            ...
            {filteredPeople.length > 0 ? (
              filteredPeople.map((person) => (
                <article key={person.name} className="rounded-[2rem] border border-white/10 bg-white/10 p-6">
                  <div className="grid gap-5 sm:grid-cols-[7rem_1fr_auto]">
                    <div className="flex h-28 w-28 items-center justify-center rounded-[1.75rem] border border-white/10 bg-gradient-to-br from-cyan-200/90 to-blue-500/90 text-3xl font-black text-slate-950 shadow-2xl shadow-blue-950/30">
                    ...
                    <div className="h-fit rounded-full bg-yellow-300/10 px-4 py-2 text-sm text-yellow-100">★ {person.rating}</div>
                  </div>
